...
function Charges(){
  const[charge,SetcData]=useState({});
  function submitHandler(event){
    ...
    axios({
      ...
      url: `http://localhost:8080/CustomizePlan/charges/charge/${val}`
        ...
        this.setState({ cData: response.data.data });
        localStorage.setItem("cdata",JSON.stringify(response.data.data));
...
function submitHandle(event){
  ...
  axios({
    ...
    return(
      ...
      <Button variant="outline-success" onClick={<ViewCharges/>} href="/charge">Search</Button>
    </Form>
    <br></br>
    <Form inline>
